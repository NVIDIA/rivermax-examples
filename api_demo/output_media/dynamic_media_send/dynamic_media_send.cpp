...
void RmxDynamicMediaSendDemoApp::add_cli_options()
{
    RmxOutMediaAPIBaseDemoApp::add_cli_options();
    
    // Remove --smpte-standard since this app is ancillary-only
    auto parser = m_cli_parser_manager->get_parser();
    auto smpte_standard_opt = parser->get_option_no_throw("--smpte-standard");
    if (smpte_standard_opt) {
        parser->remove_option(smpte_standard_opt);
    }
    m_smpte_standard = SMPTEStandard::ST_2110_40;
}

ReturnStatus RmxDynamicMediaSendDemoApp::operator()()
{
    /* Initialize settings */
    ...
    // Dynamic streams do not set packet layout - packet sizes are set per-chunk
    ...
    std::uniform_int_distribution<size_t> packet_count_dist(MIN_PACKETS_PER_CHUNK, MAX_PACKETS_PER_CHUNK);
    std::uniform_int_distribution<uint16_t> packet_size_dist(MIN_PACKET_PAYLOAD_SIZE, static_cast<uint16_t>(data_stride_size));
    ...
    while (likely(status == RMX_OK && SignalHandler::get_received_signal() < 0)) {
        ...
        do {
            /*** Set dynamic packet count for this chunk ***/
            /**
             * @note For this example, we use a random number of packets per chunk to demonstrate
             * dynamic packet count behavior. In a real application, this would be based on the
             * actual ancillary data to be transmitted.
             */
            size_t packets_in_this_chunk = packet_count_dist(m_gen);
            rmx_output_media_set_chunk_packet_count(&chunk_handle, packets_in_this_chunk);
            ...
            /**
             * @note For this example, we use random packet sizes to demonstrate dynamic packet sizing.
             * In a real application, these would be based on the actual ancillary data content.
             */
            for (size_t pkt_idx = 0; pkt_idx < packets_in_this_chunk; ++pkt_idx) {
                payload_sizes_ptr[pkt_idx] = packet_size_dist(m_gen);
            }

            /**
             * @note Fill the chunk with RTP headers and ancillary data payload based on SMPTE 2110-40 standards.
...
{
    return RmxDynamicMediaSendDemoApp().run(argc, argv);
}
