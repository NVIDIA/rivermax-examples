...
#include "drain_detached_flow_receive.h"
...
RmxDrainDetachedFlowDemoApp::RmxDrainDetachedFlowDemoApp() :
    RmxAPIBaseDemoApp(APP_DESCRIPTION, APP_EXAMPLES)
{
    std::memset(&m_source_address, 0, sizeof(m_source_address));
}

void RmxDrainDetachedFlowDemoApp::add_cli_options()
{
...
void RmxDrainDetachedFlowDemoApp::post_cli_parse_initialization()
{
    ReturnStatus status;
...
ReturnStatus RmxDrainDetachedFlowDemoApp::receive_packets(rmx_stream_id stream_id, size_t sub_block_id, size_t data_stride_size_bytes)
{
...
ReturnStatus RmxDrainDetachedFlowDemoApp::operator()()
{
    /* Initialize settings */

    /** Initialize app settings (@ref m_app_settings) **/
    // Done in @ref RmxDrainDetachedFlowDemoApp::post_cli_parse_initialization, called from @ref RmxAPIBaseDemoApp::initialize.
...
{
    return RmxDrainDetachedFlowDemoApp().run(argc, argv);
}
