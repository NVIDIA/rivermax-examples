 ...
 */
#ifndef API_DEMO_INPUT_MEMORY_REGISTRATION_RECEIVE_H_
#define API_DEMO_INPUT_MEMORY_REGISTRATION_RECEIVE_H_
...
constexpr const char* APP_EXAMPLES = \
    ...
    "  2. rmx_memory_registration_receive_demo_app --local-ip 1.2.3.4 --dst-ip 231.1.2.3 --dst-port 60000";

/**
 * @brief Basic Rivermax API Receive Demo Application With Memory Registration.
 *
 * This application demonstrates an example of explicit memory allocation and registration
 * on application side when using the Rivermax Input API.
 ...
 */
class RmxMemoryRegistrationReceiveDemoApp : public RmxInAPIBaseDemoApp
{
public:
    RmxMemoryRegistrationReceiveDemoApp() : RmxInAPIBaseDemoApp(APP_DESCRIPTION, APP_EXAMPLES) {}
    virtual ~RmxMemoryRegistrationReceiveDemoApp() = default;
protected:
    virtual ReturnStatus operator()() override;
};

#endif // API_DEMO_INPUT_MEMORY_REGISTRATION_RECEIVE_H_
